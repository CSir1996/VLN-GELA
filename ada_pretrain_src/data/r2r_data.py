...
class MultiStepNavData(object):
            ...
            with open(traj_file, 'r') as f:
                for item in jsonlines.Reader(f):
                    ...
                    for j in range(len(item['instr_encodings'])):
                        ...
                        if traj_file.endswith('gel.jsonl'):
                            for s, panos in enumerate(item['landmark_source_pano_indices'][j]):
                                span_num += 1
                                assert len(item['text_tokenid_indices'][j][s])==len(item['instr_encodings'][j])
                                if len(item['text_tokenid_indices'][j][s]) < self.max_txt_len:
                                    self.traj_box_refer.extend([(n, j, s, b, p) for b,p in enumerate(panos)])
                                elif np.array(item['text_tokenid_indices'][j][s][:self.max_txt_len]).sum() > 0:                                      
                                    # n path，j instr，s span，b pano，p pano_index
                                    self.traj_box_refer.extend([(n, j, s, b, p) for b,p in enumerate(panos)])
                        ins += 1
                    n += 1
            print("data_num:", n, ins, len(self.traj_box_refer), span_num)
    ...
    def get_input(
        ...
        ob_cand_pano_view=None, return_box = False
    ):
        ...
        if return_box:
            # text spans
            outs['text_spans'] = traj_data['text_spans'][j_instr]
            outs['text_tokenid_indices'] = traj_data['text_tokenid_indices'][j_instr]
            # bbox
            outs['landmark_bbox'] = traj_data['landmark_bbox_coords'][j_instr]
            outs['landmark_patch'] = traj_data['landmark_patches_coords'][j_instr]
            outs['landmark_source'] = traj_data['landmark_source_panos'][j_instr]
            outs['landmark_source_indices'] = traj_data['landmark_source_pano_indices'][j_instr]
