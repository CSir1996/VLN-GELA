...
############### Grounding entity-landmark ###############

class GelDataset(Dataset):
    def __init__(self, nav_db, tok, max_txt_len):
        self.nav_db = nav_db
        self.tok = tok
    ...
    def __len__(self):
        return len(self.nav_db.traj_box_refer)

    def __getitem__(self, i):
        i_traj, j_instr, span_id, box_id, path_len = self.nav_db.traj_box_refer[i]
        ...
        # prepare labels
        # text spans
        output['text_span'] = inputs['text_spans'][span_id]
        output['span_gt'] = torch.FloatTensor(inputs['text_tokenid_indices'][span_id][:self.max_txt_len])
        output['landmark_bbox'] = torch.FloatTensor(inputs['landmark_bbox'][span_id][box_id])
        output['landmark_gt'] = torch.FloatTensor(inputs['landmark_patch'][span_id][box_id] + [0])   

        return output

def gel_collate(inputs):
    batch = {
        k: [x[k] for x in inputs] for k in inputs[0].keys()
    }
    # text batches
    ...
    # labels
    # text spans
    batch['span_gt'] = pad_sequence(batch['span_gt'], batch_first=True, padding_value=0)

    # bbox
    batch['landmark_bbox'] = pad_sequence(batch['landmark_bbox'], batch_first=True, padding_value=0)
    batch['landmark_gt'] = pad_sequence(batch['landmark_gt'], batch_first=True, padding_value=0)
